...
class FlowController extends EventHandler {
  ...
  onMediaAttached(data) {
    console.log("flow-controller onMediaAttached");
    if (data.websocketName != undefined){
      var client = new WebSocket('ws://' + data.serverIp + ":" + data.serverPort + data.websocketName);
      this.wfs.attachWebsocket(client,data);
    }else{
